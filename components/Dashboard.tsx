  ...
  return (
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-6 duration-700 pb-24">
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          <h2 className="text-4xl font-black text-slate-900 dark:text-slate-100 tracking-tight transition-colors">Performance Hub</h2>
          <p className="text-slate-500 dark:text-slate-400 font-medium mt-1">Bem-vindo ao comando central dos seus estudos.</p>
        </div>
        <div className="flex items-center gap-4 bg-white dark:bg-slate-900 px-6 py-3 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm transition-colors">
          <div className="p-2.5 bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600 dark:text-indigo-400 rounded-xl transition-colors">
            <Zap size={22} />
          </div>
          <div>
            ...
            <p className="text-lg font-black text-slate-900 dark:text-slate-100 transition-colors">{totalHours} Horas</p>
          </div>
        </div>
      </header>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 bg-white dark:bg-slate-900 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 shadow-xl p-10 relative overflow-hidden group transition-colors">
          <div className="relative flex flex-col md:flex-row items-center gap-12">
            <div className="relative w-56 h-56 shrink-0 flex items-center justify-center">
               <svg className="w-full h-full transform -rotate-90" viewBox="0 0 192 192">
                 <circle cx="96" cy="96" r="82" className="stroke-slate-50 dark:stroke-slate-800 fill-none transition-colors" strokeWidth="16" />
                 <circle 
                   cx="96" cy="96" r="82" 
               ...
               <div className="absolute inset-0 flex flex-col items-center justify-center text-center">
                 <span className="text-5xl font-black text-slate-900 dark:text-slate-100 transition-colors">{weeklyGoalPercent}%</span>
            ...
            <div className="flex-1 space-y-8">
               <div>
                  <div className="flex items-center justify-between mb-3">
                    <h3 className="text-2xl font-black text-slate-900 dark:text-slate-100 flex items-center gap-3 transition-colors">
               ...
               {isEditingGoal && (
                 <div className="flex gap-3 animate-in slide-in-from-top-2">
                    <input type="number" className="flex-1 px-5 py-3 border-2 border-slate-100 dark:border-slate-800 rounded-2xl outline-none bg-slate-50 dark:bg-slate-950 dark:text-white font-bold transition-colors" value={tempGoal} onChange={(e) => setTempGoal(parseInt(e.target.value) || 0)} />
        ...
        <div className="bg-white dark:bg-slate-900 p-10 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 shadow-sm flex flex-col items-center justify-center text-center transition-colors">
            <h3 className="text-xl font-black text-slate-900 dark:text-slate-100 mb-8 transition-colors">Edital Concluído</h3>
            ...
            <div className="mt-8">
              <p className="text-4xl font-black text-slate-900 dark:text-slate-100 transition-colors">{progressPercent}%</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="bg-white dark:bg-slate-900 p-10 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 shadow-sm transition-colors">
          <h3 className="text-xl font-black text-slate-900 dark:text-slate-100 mb-8 transition-colors">Evolução em Simulados (%)</h3>
        ...
        <div className="bg-white dark:bg-slate-900 p-10 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 shadow-sm transition-colors overflow-y-auto max-h-[420px]">
          <h3 className="text-xl font-black text-slate-900 dark:text-slate-100 mb-8 transition-colors">Domínio por Disciplina</h3>
