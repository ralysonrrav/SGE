...
import { 
  ...
  BookOpen, TrendingUp, Save, Check, Database, ArrowRight, Target, Clock
} from 'lucide-react';
    ...
    <div className="space-y-6 animate-in fade-in duration-500 pb-24">
                ...
                <div className="px-6 pb-6 pt-2 border-t border-slate-50 dark:border-slate-800 space-y-3">
                   ...
                   {subject.topics.map(topic => (
                     <div key={topic.id} className="flex items-center gap-4 p-4 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-100 dark:border-slate-800 group">
                       <button onClick={() => toggleTopic(String(subject.id), topic.id)} className={`transition-all ${topic.completed ? 'text-emerald-500' : 'text-slate-300'}`}><CheckCircle size={22} /></button>
                       <span className={`flex-1 text-sm font-bold ${topic.completed ? 'text-slate-400 line-through' : 'text-slate-700 dark:text-white'}`}>{topic.title}</span>
                       <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                         <button onClick={() => { setActiveProgressTopic(topic.id); setProgHours(0); setProgMinutes(0); }} className="px-3 py-1.5 bg-white dark:bg-slate-900 text-slate-500 rounded-lg text-[10px] font-black border border-slate-100 dark:border-slate-800 hover:border-indigo-500"><TrendingUp size={14} /></button>
                         <button onClick={async () => await handleSubjectUpdate(subject.id, subject.topics.filter(t => t.id !== topic.id))} className="p-2 text-rose-500 hover:bg-rose-50 rounded-lg"><Trash2 size={16} /></button>
                       </div>
                     </div>
                   ))}
                </div>
      ...
      {activeProgressTopic && (
        <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-900 w-full max-w-md rounded-[2.5rem] p-10 shadow-2xl">
            <h3 className="text-xl font-black mb-6 dark:text-white">Registrar Estudo</h3>
            <div className="space-y-4">
              <input type="date" className="w-full px-4 py-3 rounded-xl border dark:bg-slate-950 dark:text-white" value={progDate} onChange={(e) => setProgDate(e.target.value)} />
              <div className="grid grid-cols-2 gap-4">
                <input type="number" placeholder="Horas" className="px-4 py-3 rounded-xl border dark:bg-slate-950 dark:text-white" value={progHours} onChange={(e) => setProgHours(parseInt(e.target.value) || 0)} />
                <input type="number" placeholder="Minutos" className="px-4 py-3 rounded-xl border dark:bg-slate-950 dark:text-white" value={progMinutes} onChange={(e) => setProgMinutes(parseInt(e.target.value) || 0)} />
              </div>
            </div>
            <div className="mt-8 flex gap-3">
              ...
              }} className="flex-1 bg-indigo-600 text-white font-black py-4 rounded-2xl">SALVAR</button>
              <button onClick={() => setActiveProgressTopic(null)} className="flex-1 text-slate-500 font-bold">CANCELAR</button>
            </div>
          </div>
        </div>
      )}
      ...
      {isBulkModalOpen && (
         <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md">
            <div className="bg-white dark:bg-slate-900 w-full max-w-2xl rounded-[2.5rem] p-10 shadow-2xl border dark:border-slate-800">
               <h3 className="text-xl font-black mb-6 dark:text-white">Importação em Massa</h3>
               <textarea className="w-full h-64 p-5 rounded-2xl border-2 dark:bg-slate-950 dark:text-white outline-none focus:ring-2 focus:ring-indigo-500" placeholder="Cole os tópicos linha por linha..." value={bulkText} onChange={(e) => setBulkText(e.target.value)} />
               <div className="mt-6 flex justify-end gap-3">
                  <button onClick={() => setIsBulkModalOpen(false)} className="px-6 py-3 font-bold text-slate-500">Cancelar</button>
                  <button onClick={handleBulkImport} className="px-8 py-3 bg-indigo-600 text-white font-black rounded-xl">IMPORTAR</button>
               </div>
            </div>
         </div>
      )}
